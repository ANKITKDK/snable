    ...
    const DateTimeShortcuts = {
        ...
        openClock: function(num) {
            ...
            if (window.getComputedStyle(document.body).direction !== 'rtl') {
                clock_box.style.left = findPosX(clock_link) + 17 + 'rem';
            }
            else {
                // since style's width is in em, it'd be tough to calculate
                // rem value of it. let's use an estimated rem for now
                clock_box.style.left = findPosX(clock_link) - 110 + 'rem';
            }
            clock_box.style.top = Math.max(0, findPosY(clock_link) - 30) + 'rem';
        ...
        openCalendar: function(num) {
            ...
            // is it left-to-right or right-to-left layout ?
            if (window.getComputedStyle(document.body).direction !== 'rtl') {
                cal_box.style.left = findPosX(cal_link) + 17 + 'rem';
            }
            else {
                // since style's width is in em, it'd be tough to calculate
                // rem value of it. let's use an estimated rem for now
                cal_box.style.left = findPosX(cal_link) - 180 + 'rem';
            }
            cal_box.style.top = Math.max(0, findPosY(cal_link) - 75) + 'rem';
